...
const registryVariables = {
  ...
  staging: {
    origin: "STAGING_APP_ORIGIN",
    projectRef: "STAGING_SUPABASE_PROJECT_REF",
  },
};
const loopbackHosts = new Set(["127.0.0.1", "localhost", "::1"]);
const deviceTestHostnamePattern =
...
export function registryLifecycleForDeployment(
  deploymentClass,
  releaseProfile = activeReleaseProfile,
) {
  const definition = contract.deploymentClasses[deploymentClass];
  if (!definition) return null;
  if (deploymentClass !== "PRODUCTION_RELEASE") return definition.registryLifecycle;
  return contract.releaseProfiles[releaseProfile]?.productionReleaseRegistryLifecycle ?? null;
}
...
export function validateDeploymentEnvironment(environment) {
  ...
  if (!appEnvironment || !(appEnvironment in appEnvironments)) {
    return {
      errors: [
        "APP_ENVIRONMENT must explicitly identify local, test, device-test, preview, staging, or production.",
      ],
      ok: false,
    };
  }
  ...
  const deploymentDefinition = contract.deploymentClasses[deploymentClass];
  const requiredNames = [
  ...
  if (hostedEnvironments.has(appEnvironment)) {
    requiredNames.push(
      ...
      "VERCEL_PROJECT_ID",
      "VERCEL_TARGET_ENV",
      rule.originProviderVariable,
    );

    if (rule.deploymentClasses.includes(deploymentClass) && deploymentDefinition) {
      const registryLifecycle = registryLifecycleForDeployment(deploymentClass);
      for (const registryEnvironment of registryLifecycle.requiredRegistryEnvironments) {
        requiredNames.push(
          registryVariables[registryEnvironment].projectRef,
          registryVariables[registryEnvironment].origin,
        );
      }
      for (const registryEnvironment of registryLifecycle.optionalRegistryEnvironments) {
        const registry = registryVariables[registryEnvironment];
        if (hasValue(environment, registry.projectRef) || hasValue(environment, registry.origin)) {
          requiredNames.push(registry.projectRef, registry.origin);
        }
      }
    }
  }
  ...
  if (hostedEnvironments.has(appEnvironment)) {
    const registryEntries = Object.fromEntries(
      Object.entries(registryVariables).map(([registryEnvironment, variables]) => {
        const registryProjectRef = valueOf(environment, variables.projectRef);
        const registryOriginValue = valueOf(environment, variables.origin);
        return [
          registryEnvironment,
          {
            origin: registryOriginValue
              ? parseOrigin(registryOriginValue, variables.origin, errors)
              : null,
            projectRef: registryProjectRef,
            supplied: registryProjectRef !== null || registryOriginValue !== null,
            variables,
          },
        ];
      }),
    ...
    for (const entry of Object.values(registryEntries)) {
      if (!entry.supplied) continue;
      if (!projectRefPattern.test(entry.projectRef ?? "")) {
        errors.push(
          `${entry.variables.projectRef} must be 8-40 lowercase letters or digits when supplied.`,
        );
      }
      if (
        entry.origin &&
        (entry.origin.protocol !== "https:" || loopbackHosts.has(entry.origin.hostname))
      ) {
        errors.push(
          `${entry.variables.origin} must use HTTPS and a non-loopback host when supplied.`,
        );
      }
    }

    const suppliedRegistryEntries = Object.values(registryEntries).filter(
      (entry) => entry.supplied,
    );
    const suppliedProjectRefs = suppliedRegistryEntries
      .map((entry) => entry.projectRef)
      .filter(Boolean);
    const suppliedOrigins = suppliedRegistryEntries
      .map((entry) => entry.origin?.origin)
      .filter(Boolean);
    if (
      new Set(suppliedOrigins).size !== suppliedOrigins.length
    ) {
      errors.push("All supplied hosted application origins must be distinct.");
    }
    if (new Set(suppliedProjectRefs).size !== suppliedProjectRefs.length) {
      errors.push("All supplied hosted Supabase project references must be distinct.");
    }

    const expectedAppOrigin = registryEntries[appEnvironment].origin;
    ...
    const expectedProjectRef = registryEntries[appEnvironment].projectRef;
    ...
    if (
      ...
      projectRef === registryEntries.production.projectRef
    ) {
    ...
    for (const name of [
      ...
      if (name !== providerOriginVariable && hasValue(environment, name)) {
    ...
    if (appEnvironment === "device-test") {
      if (
        appOrigin &&
        (appOrigin.protocol !== "https:" ||
          appOrigin.port !== "" ||
          !deviceTestHostnamePattern.test(appOrigin.hostname) ||
          valueOf(environment, "APP_ORIGIN") !== appOrigin.origin)
      ) {
        errors.push("Device-test APP_ORIGIN must be one exact HTTPS node.tailnet.ts.net origin on port 443.");
      }
      if (environment.NODE_ENV !== "production") {
        errors.push("APP_ENVIRONMENT=device-test requires NODE_ENV=production for secure Auth cookies.");
      }
    } else if (appOrigin && !loopbackHosts.has(appOrigin.hostname)) {
      errors.push("Local/test APP_ORIGIN must be loopback-only.");
    }
    if (supabaseUrl && !loopbackHosts.has(supabaseUrl.hostname.replace(/^\[|\]$/g, ""))) {
      errors.push("Local/test/device-test NEXT_PUBLIC_SUPABASE_URL must be loopback-only.");
    }
    if (projectRef !== "local") {
      errors.push("Local/test/device-test SUPABASE_PROJECT_REF must be local.");
    }
    for (const name of [
    ...
    if (appEnvironment === "device-test") {
      for (const name of Object.keys(environment)) {
        if ((name === "VERCEL" || name.startsWith("VERCEL_")) && hasValue(environment, name)) {
          errors.push(`${name} contradicts APP_ENVIRONMENT=device-test.`);
        }
      }
    }
  ...
  return {
    appEnvironment,
    appOrigin: appOrigin?.origin ?? null,
    errors,
    ok: errors.length === 0,
    releaseProfile: activeReleaseProfile,
    supabaseEnvironment,
