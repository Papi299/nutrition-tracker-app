...
type ConfirmationErrorPageProps = Readonly<{
...
export default async function ConfirmationErrorPage({
  params,
}: ConfirmationErrorPageProps) {
  const { locale } = await params;

  setRequestLocale(locale);

  return <LocalizedConfirmationError locale={locale} />;
}

function LocalizedConfirmationError({ locale }: { locale: Locale }) {
  const t = useTranslations("Auth.confirmationError");
