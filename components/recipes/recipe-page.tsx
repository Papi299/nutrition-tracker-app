...
export function RecipeEditorPageHeader({
  children,
  mode,
}: {
...
export function RecipeRetrievalError({
  locale,
  retryHref,
}: {
  locale: Locale;
  ...
  return (
    <section className="flex flex-1 flex-col justify-center py-8 text-start">
      <RetrievalError
        ...
        retryHref={retryHref}
        retryLabel={t("retry")}
        testId="recipe-retrieval-error"
        title={t("failureTitle")}
      />
      <Link className="mt-4 text-sm font-semibold text-teal-800 underline" href={`/${locale}/recipes`}>
        {t("back")}
      </Link>
    </section>
  );
